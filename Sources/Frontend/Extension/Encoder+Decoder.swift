...
extension FoundationEssentials.JSONEncoder: HBResponseEncoder {
  /// Extend FoundationEssentials.JSONEncoder to support encoding `HBResponse`'s. Sets body and header values
  /// - Parameters:
...
extension FoundationEssentials.JSONDecoder: HBRequestDecoder {
  ...
  public func decode<T: Decodable>(_ type: T.Type, from request: HBRequest) throws -> T {
    ...
    let foundationData: Data = data.withUnsafeBytes { buff in
      return Data(bytes: buff, count: buffer.readableBytes)
    }
